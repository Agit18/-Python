...
while escape == 0:
    ...
    from_lang_var.set(language1)  # По умолчанию переводим с английского языка
    from_lang_menu = OptionMenu(Start, from_lang_var, "en", "ru", "fr", "es")  # Выберите языки, которые вам нужны
    from_lang_menu.pack()
    to_lang_var = StringVar()
    to_lang_var.set(language2)  # По умолчанию переводим на русский язык
    to_lang_menu = OptionMenu(Start, to_lang_var, "ru", "en", "fr", "es")  # Выберите языки, которые вам нужны
    to_lang_menu.pack()
    Start.lift()
    Start.title('')
    ...
    if escape == 0:
        ...
        pole = Text(root, width=80, height=20, font='Arial, 9')
        pole.pack(pady=10) 
        mainphoto = PhotoImage(file = "im\images.png")
